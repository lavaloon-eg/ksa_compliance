...
class InvoiceLineFactory(Factory):
    ...
    def create(zatca_fields_dto: dict, doc):
        ...
        """
        invoice_lines = [
            InvoiceLineFactory._create_prepayment_invoice_line(advance, doc)
    ...
    def _create_prepayment_invoice_line(advance, doc):
        ...
        prepayment_invoice_line_builder.set_item(
            InvoiceLineFactory._get_item(advance, doc, document_reference)
        )
        prepayment_invoice_line_builder.set_price(
            InvoiceLineFactory._get_price(advance, doc, document_reference)
        )
    ...
    def _get_invoice_quantity_xml_tag(invoice_quantity: float, currency: str) -> XMLTag:
        return XMLTagFactory.create(
            "InvoiceQuantity",
            attributes={"unitCode": currency},
            text=float(invoice_quantity)
        )
    @staticmethod
    ...
    def _get_document_reference(advance, doc, document_reference):
        ...
        document_reference_builder.set_id(
    ...
    def _get_tax_total(advance, doc, document_reference):
        ...
        tax_id = frappe.db.get_value(
            ...
            document_reference.sales_taxes_and_charges_template,
            "tax_category"
        )
    ...
    def _get_tax_subtotal(advance, doc, tax_percent, tax_category_id):
        ...
        tax_subtotal.set_taxable_amount(
    ...
    def _get_item(advance, doc, document_reference):
        ...
        item = ItemBuilder()
        item.set_name(
            document_reference.custom_prepayment_invoice_description
        ).set_item_tax_category(
            tax_category_id.tax_category_code
        ).set_item_tax_percent(
            tax_percent
        ).set_item_tax_scheme(
            "VAT"
        )
