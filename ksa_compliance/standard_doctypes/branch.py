...
def validate_mandatory_crn(doc):
    if doc.custom_branch_ids and doc.custom_company and ZATCABusinessSettings.is_branch_config_enabled(doc.custom_company):
        crn = doc.custom_branch_ids[0].value
        crn = crn.strip() or None if type(crn) == str else crn
        if not crn:
            fthrow(msg=_('CRN is mandatory when ZATCA branch configuration is enabled for company.'), title=_('Mandatory CRN'))


def validate_duplicate_crn(doc):
    if doc.custom_branch_ids:
        crn = doc.custom_branch_ids[0].value
        if crn and frappe.db.exists("Additional Seller IDs", {
