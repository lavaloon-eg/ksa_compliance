...
class SalesInvoiceAdditionalFields(Document):
    ...
    def prepare_for_zatca(self, settings: ZATCABusinessSettings):
        ...
        result = cli.sign_invoice(settings.zatca_path, invoice_xml, cert_path, settings.private_key_path)

        if settings.validate_generated_xml and not self.is_compliance_mode:
            validation_result = cli.validate_invoice(settings.zatca_path, result.signed_invoice_path,
